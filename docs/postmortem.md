...
### Timeline
- **02:22:37** — Memory stress injected inside worker-service pod (simulating a memory leak) via `python3` script allocating unbounded memory.
...
### Root Cause
Worker-service was given a memory limit of 128Mi. Simulated workload (memory leak pattern) exceeded this limit, causing Kubernetes' OOM killer to terminate the container to protect the node from memory exhaustion.
...
- Old, healthy green pod continued serving all traffic — zero downtime, zero bad requests reached users.
...
### Fix
Reverted the `/health` endpoint to return `{"status": "healthy"}, 200`. Rebuilt and reloaded the image, then restarted the deployment. New pod passed readiness checks and rejoined the Service.
...
- Blue-green strategy (built in M7) meant this bad deploy caused **zero user-facing impact** — old version kept serving throughout. This is the core value of blue-green over a rolling update, where a bad pod might briefly receive live traffic.
